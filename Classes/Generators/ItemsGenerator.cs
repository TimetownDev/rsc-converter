...
public class ItemsGenerator : IClassGenerator
{
    ...
    public IList<ClassDefinition>? OnGenerate(BuildSession session)
    {
        ...
        if (generators is not YamlMappingNode generatorsMappingNode) return null;
        foreach (KeyValuePair<YamlNode, YamlNode> pair in generatorsMappingNode)
        {
            YamlNode key = pair.Key;
            if (key is not YamlScalarNode scalarNode) continue;
            string? stringKey = scalarNode.Value;
        ...
        if (solarGenerators is not YamlMappingNode solarGeneratorsMappingNode) return null;
        foreach (KeyValuePair<YamlNode, YamlNode> pair in solarGeneratorsMappingNode)
        {
            YamlNode key = pair.Key;
            if (key is not YamlScalarNode scalarNode) continue;
            string? stringKey = scalarNode.Value;
